...
1. 验证资源的完整性
   ...
   match -seed /home/ec2-user/double_hit/assets_config/common_android/project.manifest -root /home/ec2-user/double_hit/v${version}/res_doublehit/
   match -seed /home/ec2-user/double_hit/assets_config/common_ios/project.manifest -root /home/ec2-user/double_hit/v${version}/res_doublehit/
   match_version.sh double_hit v${version}

2. 同步faceBook资源
   cd /usr/share/nginx/https
   sh dh_pubfbclient.sh double_hit v${version}

3. 同步native资源
   cd /usr/share/nginx/https
   sh dh_pubclient.sh double_hit v${version}

4. 设置reuse版本
   cd /home/ec2-user/double_hit
   mv v${version} reuse_version
   cd /home/ec2-user/double_hit_fb
   mv v${version} reuse_version

5. 设置nginx_reuse版本
   cd /usr/share/nginx/https/double_hit
   mv v${reuse_version} reuse_version
   cd /usr/share/nginx/https/double_hit_fb
   mv v${reuse_version} reuse_version
...
step1() {
    ...
    if [ $? -eq 0 ]; then
        ...
        # 在后台运行match命令
        ./match -seed /home/ec2-user/double_hit/assets_config/common_android/project.manifest \
            -root /home/ec2-user/double_hit/v${VERSION}/res_doublehit/ &
        MATCH_PID=$!
    ...
    if [ $? -eq 0 ]; then
        ...
        # 在后台运行match命令
        ./match -seed /home/ec2-user/double_hit/assets_config/common_ios/project.manifest \
            -root /home/ec2-user/double_hit/v${VERSION}/res_doublehit/ &
        MATCH_PID=$!
    ...
    if [ $? -eq 0 ]; then
        ...
        sh match_version.sh double_hit v${VERSION}
...
step2() {
    ...
    wait_continue "执行 dh_pubfbclient.sh 同步Facebook资源"
    if [ $? -eq 1 ]; then
        print_warning "跳过步骤2"
        return
    fi

    print_info "切换到目录: /usr/share/nginx/https"
    cd /usr/share/nginx/https || {
        print_error "无法切换到 /usr/share/nginx/https 目录"
        return 1
    }

    print_info "执行: sh dh_pubfbclient.sh double_hit v${VERSION}"
    sh dh_pubfbclient.sh double_hit v${VERSION}
...
step3() {
    ...
    wait_continue "执行 dh_pubclient.sh 同步Native资源"
    if [ $? -eq 1 ]; then
        print_warning "跳过步骤3"
        return
    fi
    ...
    print_info "执行: sh dh_pubclient.sh double_hit v${VERSION}"
    print_warning "此脚本需要交互输入,请根据提示输入 yes 等信息..."
    echo ""
...
step4() {
    ...
    wait_continue "重命名 double_hit 和 double_hit_fb 目录中的 v${VERSION}"
    if [ $? -eq 1 ]; then
        print_warning "跳过步骤4"
        return
    fi
    ...
    # double_hit目录
    print_info "切换到目录: /home/ec2-user/double_hit"
    cd /home/ec2-user/double_hit || {
        print_error "无法切换到 /home/ec2-user/double_hit 目录"
        return 1
    }
    ...
    if [ $? -ne 0 ]; then
        print_error "/home/ec2-user/double_hit: v${VERSION} 重命名失败"
        return 1
    fi

    # double_hit_fb目录
    print_info "切换到目录: /home/ec2-user/double_hit_fb"
    cd /home/ec2-user/double_hit_fb || {
        print_error "无法切换到 /home/ec2-user/double_hit_fb 目录"
        return 1
    }
...
step5() {
    ...
    wait_continue "重命名 nginx/double_hit 和 nginx/double_hit_fb 目录中的 v${REUSE_VERSION}"
    if [ $? -eq 1 ]; then
        print_warning "跳过步骤5"
        return
    fi
    ...
    # double_hit目录
    print_info "切换到目录: /usr/share/nginx/https/double_hit"
    cd /usr/share/nginx/https/double_hit || {
        print_error "无法切换到 /usr/share/nginx/https/double_hit 目录"
        return 1
    }
    ...
    if [ $? -ne 0 ]; then
        print_error "/usr/share/nginx/https/double_hit: v${REUSE_VERSION} 重命名失败"
        return 1
    fi

    # double_hit_fb目录
    print_info "切换到目录: /usr/share/nginx/https/double_hit_fb"
    cd /usr/share/nginx/https/double_hit_fb || {
        print_error "无法切换到 /usr/share/nginx/https/double_hit_fb 目录"
        return 1
    }
